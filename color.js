...
Color.complex = function (z) {
  ...
  a = 1 - 1 / (a * inv_scale + 1);
  ans[0] =
    0.6 + 0.3 * x + 0.35 * (1 - a) * Math.pow(0.5 + 0.4 * y + 0.3 * x, 2);
  ans[1] =
    0.5 + (0.45 + 0.05 * a) * (-0.8 * x + 0.6 * y) + 0.05 * x * x * (1 - a);
  ...
  for (var i in ans) {
    ans[i] = Math.min(Math.max(0, ans[i]), 1);
  }
  var l =
    0.11 * ans[0] +
    0.55 * ans[1] +
  ...
  if (a < kthresh) {
    var k = a / kthresh;
    k =
      (Math.pow(2.5 - 2 * l + k, 2) - Math.pow(2.5 - 2 * l, 2)) /
  ...
  if (a > l) {
    var w = (a - l) / (1 - l);
    w =
      (Math.pow(1.1 - l + w, 2) - Math.pow(1.1 - l, 2)) /
  ...
  for (var i in ans) {
    ans[i] = (4 - Math.pow(2 - ans[i], 2)) * 0.333333;
  }
  for (var i in ans) {
    ans[i] = Math.round(ans[i] * 255);
  }
  return ans;
};
